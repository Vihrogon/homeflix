...
Deno.test("html function handles functional components correctly", () => {
    function component(params) {
        return html`<ul>${params.map((param) => html`<li>${param}</li>`)}</ul>`;
    }
    const items = ["test1", "test2"];
    const result = html`<div>${component(items)}</div>`;
    const expected = "<ul><li>test1</li><li>test2</li><li>test3</li></ul>";
    equal(result, expected);
});
